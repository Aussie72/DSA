...
public class subSequence{
    public static void subSequenceAscii(String q, String p){
        if(q.isEmpty()){
            System.out.println(p);
            return;
        }
        char ch = q.charAt(0);
        subSequenceAscii(q.substring(1),p+ch);
        subSequenceAscii(q.substring(1),p+(ch+0));
        subSequenceAscii(q.substring(1),p);        
    }
